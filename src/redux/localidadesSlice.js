...
export const fetchNeighborhood = createAsyncThunk(
  ...
  async () => {
    try {
      ...
      const { data } = await axios.get(`/sitters`);
      console.log("Datos cargados correctamente:", data);
      return data;
    } catch (error) {
      console.error(
        "Error al obtener la información de los cuidadores:",
        error
      );
      throw error;
    }
  ...
  extraReducers: (builder) => {
    builder
      .addCase(fetchNeighborhood.fulfilled, (state, action) => {
        ...
        const allNeighborhoods = action.payload.map(
          (cuidador) => cuidador.neighborhood
        );
        ...
        action.payload.forEach((cuidador) => {
          state.sitterCount[cuidador.neighborhood] =
            (state.sitterCount[cuidador.neighborhood] || 0) + 1;
        });
      })
...
export const selectNeighborhoods = (state) =>
  state.neighborhoodSitter.neighborhoods;
export const selectSittersCount = (state) =>
  state.neighborhoodSitter.caregiversCount;
